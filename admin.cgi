...
#path to repository
my $baseurl = 'http://svn.cutt.com';
my $svnroot = '/var/lib/svn/repos';
my $viewcvspath = "/svn";
my $svnpath = "/svn";
my $user_auth = "/var/lib/svn/conf/davsvn.passwd";
my $svnaccess_conf = "/var/lib/svn/conf/davsvn.authz";
my $svnaccess_conf_perm = 0640;
my $gpgkeys = "$svnroot/.htgpgkeys";
my $gpghome = '/var/lib/wwwrun/.gnupg';
...
if ($action eq "adduser" && in_group("admin")) {
	...
	if (grep(/^$newUser$/, @htpasswd_users)) {
...
if ($repos ne "" && in_group("$repos-admins")) {
    print h2({-id=>$repos},"Administrate repository ".
	     a({href=>"${viewcvspath}/$repos/"}, $repos));
	...
	print "<tr><td>User:</td><td><input type='text' size='30' name='newUser'/></td></tr>";
	print "<tr><td>Passphrase:</td><td><input type='text' size='30' name='pwd'/></td></tr>";
	print "<tr><td>Verify-Passphrase:</td><td><input type='text' size='30' name='vpwd'/></td></tr>";
...
# 	print hr;
	...
	foreach $repos (sort keys %repositories) {
		...
		foreach $key (sort keys %{$repositories{$repos}{$path}}) {
			...
			|| $key eq '*'
			|| ($key =~ /^@(.*)$/ && in_group("$1"))) {
			print li(a({href => "${viewcvspath}/$repos$path"}, 
...
print p("<small>Copyright 2005&ndash;2007 Jochen Hoenicke, Michael M&ouml;ller, Marco L&uuml;bcke; based on code from Theo Van Dinter</small>"),"\n";
print end_html,"\n";
